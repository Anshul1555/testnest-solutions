...
export default function Home({ onOpenAuditModal, setActiveTab, onCopy, copiedState }) {
  ...
  const processSteps = [
    { num: '01', title: 'Discovery Call', desc: 'We learn your product, stack, pain points, and release cadence in a free 30-min session.' },
    { num: '02', title: 'QA Strategy', desc: 'Custom test plan tailored to your risk areas — scope, tools, types, and timeline defined clearly.' },
    { num: '03', title: 'Execution Sprint', desc: 'Manual and automation testing in parallel sprints aligned with your dev cycles.' },
    { num: '04', title: 'Bug Reporting', desc: 'Detailed reports with severity, steps to reproduce, screenshots, and fix recommendations.' },
    { num: '05', title: 'Sign-Off & Support', desc: 'Final regression pass, release confidence score, and ongoing QA partnership if needed.' }
  ];
  ...
  return (
    <div className="pt-24 pb-16">
      ...
      <section className="py-20 px-6 bg-white border-y border-slate-200/50" id="services">
        <div className="max-w-6xl mx-auto">
          <div className="text-center max-w-2xl mx-auto mb-16 space-y-4">
            ...
            <h2 className="text-3xl md:text-4xl font-extrabold text-slate-800 tracking-tight">What We Offer</h2>
            ...
            {services.map((item, idx) => (
              <TiltCard key={idx} className={`glass-card p-8 rounded-3xl flex flex-col items-start text-left border border-slate-200/30 bg-gradient-to-br ${item.bg} hover:shadow-xl transition-all duration-300 relative overflow-hidden group`}>
                ...
                <div className="flex justify-between items-center w-full mb-6">
                  <div className="p-3 bg-white rounded-xl shadow-md border border-slate-100">{item.icon}</div>
                  <span className="font-mono text-xs font-bold text-slate-400">{item.num}</span>
                </div>
                <h3 className="text-lg font-bold text-slate-800 mb-2"><span>{item.title}</span></h3>
                <p className="text-sm font-medium text-slate-600 leading-relaxed mb-6 flex-grow">{item.desc}</p>
                <div className="flex flex-wrap gap-2 pt-4 border-t border-slate-200/40 w-full">
                  {item.tags.map((tag, tIdx) => (
                    <span key={tIdx} className="px-2.5 py-0.5 rounded-full text-[10px] font-bold text-slate-500 bg-white shadow-sm border border-slate-200/40">{tag}</span>
                  ))}
                </div>
              </TiltCard>
            ))}
      ...
      <section className="py-20 px-6 bg-[#F4F7FA] border-b border-slate-200/50">
        <div className="max-w-6xl mx-auto">
          <div className="text-center max-w-2xl mx-auto mb-16 space-y-4">
            <span className="inline-block px-3 py-1 rounded-full text-[10px] font-bold tracking-wider uppercase text-indigo-700 bg-indigo-50 border border-indigo-100">Targeted Verticals</span>
            <h2 className="text-3xl md:text-4xl font-extrabold text-slate-800 tracking-tight">Multi-Domain QA Industry Expertise</h2>
            <p className="text-slate-600 font-medium leading-relaxed">We understand that testing parameters change by industry. TestNest features deep subject-matter knowledge across critical business sectors.</p>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            <motion.div whileHover={{ y: -6, scale: 1.01 }} className="glass-card p-8 rounded-3xl flex flex-col items-start text-left border border-slate-200/30 bg-white hover:shadow-lg transition-all duration-300 relative overflow-hidden group">
              ...
              <div className="flex justify-between items-center w-full mb-6">
                <div className="p-3 bg-emerald-50 rounded-xl shadow-md border border-emerald-100/50 text-emerald-600"><Coins className="w-6 h-6" /></div>
                <span className="font-mono text-xs font-bold text-slate-400">01</span>
              </div>
              <h3 className="text-lg font-black text-slate-800 mb-3">Fintech & Core Banking</h3>
              <p className="text-xs font-semibold text-slate-500 leading-relaxed mb-6">Assuring high-frequency transactional data models, ledger balances, secure dynamic authentication token bindings, and API gateway routing under peak concurrent threads.</p>
            ...
            <motion.div whileHover={{ y: -6, scale: 1.01 }} className="glass-card p-8 rounded-3xl flex flex-col items-start text-left border border-slate-200/30 bg-white hover:shadow-lg transition-all duration-300 relative overflow-hidden group">
              <div className="absolute top-0 right-0 w-24 h-24 bg-indigo-200/10 rounded-full blur-2xl transform translate-x-4 -translate-y-4 group-hover:scale-125 transition-transform duration-300" />
              <div className="flex justify-between items-center w-full mb-6">
                <div className="p-3 bg-indigo-50 rounded-xl shadow-md border border-indigo-100/50 text-indigo-600"><ShieldCheck className="w-6 h-6" /></div>
                <span className="font-mono text-xs font-bold text-slate-400">02</span>
              </div>
              <h3 className="text-lg font-black text-slate-800 mb-3">Finance & Insurance</h3>
              <p className="text-xs font-semibold text-slate-500 leading-relaxed mb-6">Validating complex policy calculation matrices, automated claims routing criteria, actuarial rules parameters, and secure customer portal workflows for zero compliance risk.</p>
              <div className="flex flex-wrap gap-2 pt-4 border-t border-slate-100 w-full mt-auto">
            ...
            <motion.div whileHover={{ y: -6, scale: 1.01 }} className="glass-card p-8 rounded-3xl flex flex-col items-start text-left border border-slate-200/30 bg-white hover:shadow-lg transition-all duration-300 relative overflow-hidden group">
              ...
              <div className="flex justify-between items-center w-full mb-6">
                <div className="p-3 bg-cyan-50 rounded-xl shadow-md border border-cyan-100/50 text-cyan-600"><ShoppingBag className="w-6 h-6" /></div>
                <span className="font-mono text-xs font-bold text-slate-400">03</span>
              </div>
              <h3 className="text-lg font-black text-slate-800 mb-3">Retail & E-commerce</h3>
              <p className="text-xs font-semibold text-slate-500 leading-relaxed mb-6">Testing critical customer checkout journeys, shopping cart state persistence, localized dynamic taxation logic, promo code combinations, and stock thresholds syncs.</p>
              <div className="flex flex-wrap gap-2 pt-4 border-t border-slate-100 w-full mt-auto">
      ...
      <section className="py-16 px-6 bg-slate-50 overflow-hidden border-b border-slate-200/50" id="tech">
        <div className="max-w-6xl mx-auto mb-10 text-center">
          <span className="inline-block px-3 py-1 rounded-full text-[10px] font-bold tracking-wider uppercase text-indigo-700 bg-indigo-50 border border-indigo-100 mb-3">Engineered Tooling</span>
        ...
        <div className="ticker-wrap w-full py-4 relative">
          ...
          <div className="ticker-content flex items-center">
            {tickerLogos.map((logo, index) => (
              <div key={index} className="px-6 py-3 rounded-2xl bg-white shadow-sm border border-slate-200/50 text-slate-700 font-bold text-sm tracking-wide font-mono inline-flex items-center gap-2 hover:border-indigo-300 hover:text-indigo-600 transition-all duration-300 cursor-pointer">
      ...
      <section className="py-20 px-6 bg-white border-b border-slate-200/50" id="process">
        <div className="max-w-4xl mx-auto">
          <div className="text-center mb-16 space-y-4">
            <span className="inline-block px-3 py-1 rounded-full text-[10px] font-bold tracking-wider uppercase text-indigo-700 bg-indigo-50 border border-indigo-100">Our Process</span>
            <h2 className="text-3xl md:text-4xl font-extrabold text-slate-800 tracking-tight">How It Works</h2>
            <p className="text-slate-600 font-bold text-sm uppercase tracking-wide">From first call to flawless launch</p>
          </div>
          <div className="space-y-6 text-left relative before:absolute before:top-2 before:bottom-2 before:left-[39px] before:w-[2px] before:bg-indigo-100">
            {processSteps.map((step, idx) => (
              <motion.div key={idx} initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true, margin: '-100px' }} transition={{ duration: 0.5, delay: idx * 0.1 }} className="glass-card p-6 md:p-8 rounded-3xl grid grid-cols-1 md:grid-cols-12 gap-4 md:gap-8 items-start relative border border-slate-200/40 hover:border-indigo-200 hover:shadow-md transition-all duration-300">
                <div className="md:col-span-2 flex items-center justify-start md:justify-center">
                  <div className="h-14 w-14 rounded-2xl bg-gradient-to-br from-indigo-500 to-cyan-400 text-white font-extrabold text-lg flex items-center justify-center shadow-md relative z-10">{step.num}</div>
                </div>
      ...
      <section className="py-20 px-6 bg-slate-50">
        <div className="max-w-4xl mx-auto">
          <motion.div whileHover={{ scale: 1.01 }} className="glass-card rounded-3xl p-8 md:p-12 border border-slate-200/60 bg-gradient-to-br from-white to-indigo-50/20 shadow-xl flex flex-col items-center text-center space-y-8 relative glow-indigo overflow-hidden">
            ...
            <div className="space-y-4">
              <span className="inline-block px-3 py-1 rounded-full text-[10px] font-bold tracking-wider uppercase text-indigo-700 bg-indigo-100 border border-indigo-200/50">Direct Contact Card</span>
              <h2 className="text-3xl md:text-4xl font-extrabold text-slate-800 tracking-tight">Ready to ship bug-free?</h2>
              <p className="text-slate-600 font-medium max-w-md mx-auto">Free 30-minute consultation. No commitment. Just honest QA advice for your project.</p>
            </div>
            <a href="mailto:anshulsharma9366@gmail.com?subject=TestNest QA Consultation Inquiry&body=Hi Anshul,%0D%0A%0D%0AI would like to schedule our 30-minute free QA discovery session to audit my application." className="btn btn-primary px-8 py-3.5 bg-indigo-600 hover:bg-indigo-700 text-white font-bold rounded-xl shadow-lg hover:shadow-indigo-500/20 transform hover:-translate-y-0.5 transition-all duration-300 flex items-center gap-2 cursor-pointer text-sm">
              Book Call &rarr;
            </a>
            <div className="flex flex-wrap items-stretch justify-center gap-6 w-full pt-8 border-t border-slate-200/60 text-left">
              <div className="bg-white rounded-2xl p-4 border border-slate-200/40 flex items-start gap-3 shadow-sm group flex-grow flex-shrink-0 min-w-[280px]">
                <div className="p-2.5 bg-indigo-50 rounded-xl text-indigo-600"><Mail className="w-5 h-5" /></div>
                <div className="flex-grow">
                  <span className="block text-[10px] font-bold uppercase tracking-wider text-slate-400">Email Directly</span>
                  <a href="mailto:anshulsharma9366@gmail.com" className="block text-xs font-bold text-slate-700 hover:text-indigo-600 transition-colors duration-200">anshulsharma9366@gmail.com</a>
                </div>
                <button onClick={() => onCopy('anshulsharma9366@gmail.com', 'email')} className="p-1.5 text-slate-400 hover:text-indigo-600 bg-slate-50 hover:bg-indigo-50 rounded-lg cursor-pointer transition-all" title="Copy email address">
              ...
              <div className="bg-white rounded-2xl p-4 border border-slate-200/40 flex items-start gap-3 shadow-sm group flex-grow flex-shrink-0 min-w-[240px]">
                <div className="p-2.5 bg-indigo-50 rounded-xl text-indigo-600"><Phone className="w-5 h-5" /></div>
                <div className="flex-grow">
                  <span className="block text-[10px] font-bold uppercase tracking-wider text-slate-400">Direct Call</span>
                  <a href="tel:+14379822044" className="block text-xs font-bold text-slate-700 hover:text-indigo-600 transition-colors duration-200">437-982-2044</a>
                </div>
                <button onClick={() => onCopy('4379822044', 'phone')} className="p-1.5 text-slate-400 hover:text-indigo-600 bg-slate-50 hover:bg-indigo-50 rounded-lg cursor-pointer transition-all" title="Copy phone number">
              ...
              <div className="bg-white rounded-2xl p-4 border border-slate-200/40 flex items-start gap-3 shadow-sm group flex-grow flex-shrink-0 min-w-[340px]">
                <div className="p-2.5 bg-indigo-50 rounded-xl text-indigo-600"><ExternalLink className="w-5 h-5" /></div>
                <div className="flex-grow">
                  <span className="block text-[10px] font-bold uppercase tracking-wider text-slate-400">LinkedIn Connect</span>
                  <a href="https://www.linkedin.com/in/anshul1555/" target="_blank" rel="noopener noreferrer" className="block text-xs font-bold text-slate-700 hover:text-indigo-600 transition-colors duration-200">https://www.linkedin.com/in/anshul1555/</a>
                </div>
                <button onClick={() => onCopy('https://www.linkedin.com/in/anshul1555/', 'linkedin')} className="p-1.5 text-slate-400 hover:text-indigo-600 bg-slate-50 hover:bg-indigo-50 rounded-lg cursor-pointer transition-all" title="Copy LinkedIn Link">
